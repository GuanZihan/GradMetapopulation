...
class SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, csv_file: str, target_name: str, masked_feature:str=""):
        ...
        train_days = CONFIGS['COVID']["train_days"]        
...
def runner(params: Dict[str, object], devices: Sequence[torch.device], verbose: bool, args) -> Tuple[List[torch.Tensor], np.ndarray, List[torch.Tensor]]:
    for run_id in range(params['num_runs']):
        ...
        train_dataset = SeqDataset("./Data/US/online/train_{}_dp2.csv".format(args.date), "cases", args.mask_column)
        test_dataset = SeqDataset("./Data/US/online/test_{}_dp2.csv".format(args.date), "cases", args.mask_column)
        ...
        metas_train_dim = 1
        X_train_dim = 5 if args.mask_column == "" else 4
