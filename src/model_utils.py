...
class MetapopulationSEIRM:
    ...
    def step(self, t, values, seed_status, adjustment_matrix):
        ...
        E_beta_j_eff = E_beta_j_eff * params["beta"]
...
class MetapopulationSEIRMSinglePatch:
    def __init__(self, params, device, num_patches, migration_matrix, num_agents, seed_infection_status={}):
        super().__init__()
        self.device = device
        self.num_patches = 1
        self.state = torch.zeros((self.num_patches, 5)).to(self.device)
        self.params = params
        self.migration_matrix = torch.ones((1, 1), device=self.device)
        self.num_agents = num_agents.to(self.device)
        self.num_agents = self.num_agents.sum().view(1)
        self.seed_infection_status = seed_infection_status
