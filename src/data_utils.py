...
def load_county_df(county,ew_start_data,ew_end_data,args=None):
    """ load and clean data"""
    if args is not None:
...
def get_county_train_data(county,pred_week,ew_start_data=EW_START_DATA,noise_level=0,args=None):
    # import data
    pred_week=convert_to_epiweek(pred_week) 
    ew_start_data=convert_to_epiweek(ew_start_data)
    df = load_county_df(county,ew_start_data,pred_week,args=args)
    # select targets
...
def get_time_series_train_data(age_group,noise_level=0,args=None):
    # import data
    df = pd.read_csv("Data/Processed/{}.csv".format(age_group))
    # select targets
    targets = df.loc[:,['covid-19 en colombia', 'covid-19 vacuna', 'covid-19 hoy', 'covid-19 bogota']].values
    if noise_level>0:
        # noise_level is an index for your list
        noise = NOISE_LEVELS_COVID[noise_level-1]
        std_vals = np.std(targets, axis=0)*noise
...
def get_age_group_train_data(age_group,noise_level=0,args=None,split="train"):
    # import data
    df = pd.read_csv("Data/Processed/age_group_{}.csv".format(split))
    df = df[df.Age_groups == age_group]
    # select targets
    targets = df.loc[:,['Deaths', 'Cases']].values
    if noise_level>0:
        # noise_level is an index for your list
        noise = NOISE_LEVELS_COVID[noise_level-1]
        std_vals = np.std(targets, axis=0)*noise
        ...
        noisy_targets = noisy_targets.astype('int32')
        targets = np.maximum(noisy_targets, 0)
    df.drop(columns=['Age_groups','Time'],inplace=True)
    return df, targets
