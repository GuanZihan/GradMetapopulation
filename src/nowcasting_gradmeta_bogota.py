...
class SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, csv_file, target_name, gt_file):
...
class CalibNNTwoEncoderThreeOutputs(nn.Module):
    def __init__(self, params, metas_train_dim, X_train_dim, device, training_weeks, hidden_dim=64, out_dim=1, n_layers=2, scale_output='abm-covid', bidirectional=True):
        super().__init__()
...
class CalibNNOneEncoderThreeOutputs(nn.Module):
    ...
    def forward(self, x, meta, x_2, meta2, train_X, train_Y):
        ...
        x_embeds_2, encoder_hidden_2 = self.emb_model_2.forward(x_2.transpose(1, 0), meta2)
        # concatenate embeddings
        x_embeds = x_embeds_2.mean(dim=0).unsqueeze(0)
        encoder_hidden = encoder_hidden_2.mean(dim=1).unsqueeze(1)
...
def runner(params, devices, verbose, args):
    for run_id in range(params['num_runs']):
        ...
        for file in args.revision_files:
            train_dataset_temp = SeqDataset("./Data/Processed/online/{}".format(file), "cases", "./Data/Processed/online/{}".format("_".join(file.split("_")[:-1])+"_2021-01-16.csv"))
            ...
            # test_dataset = SeqDataset("./Data/Processed/online/2020-08-29_0_moving_2020-08-29.csv".format(args.date), "cases", "./Data/Processed/online/2020-08-29_0_moving_2020-09-10.csv")
            # test_dataset = SeqDataset("./Data/Processed/online/2021-01-16_0_moving_2021-01-16.csv".format(args.date), "cases", "./Data/Processed/online/2021-01-16_0_moving_2021-02-20.csv")
            # test_dataset = SeqDataset("./Data/Processed/online/2021-01-23_0_moving_2021-01-23.csv".format(args.date), "cases", "./Data/Processed/online/2021-01-23_0_moving_2021-03-27.csv")

            # test_dataset = SeqDataset("./Data/Processed/online/2021-01-30_0_moving_2021-01-30.csv".format(args.date), "cases", "./Data/Processed/online/2021-01-30_0_moving_2021-03-27.csv")
            test_dataset = SeqDataset("./Data/Processed/online/2021-02-06_0_moving_2021-02-06.csv".format(args.date), "cases", "./Data/Processed/online/2021-02-06_0_moving_2021-03-27.csv")
        ...
        if 'learnable-params' in params['model_name']:
            ...
            lr = 1e-4 if params['model_name'].startswith('GradMeta') else 1e-4
        ...
        if train_flag:
            for tr_idx in range(len(train_loaders)):
                tqdm.write("\033[91m" + f"File: {args.revision_files[tr_idx]}" + "\033[0m")
                param_model.train()
        ...
        for state_idx, target_values in enumerate(y.unsqueeze(0)):
            ...
            # print(target.shape, predictions_train.shape, NOWCASTING_PERIOD, NOWCASTING_END)
            # print(target[-NOWCASTING_PERIOD:-NOWCASTING_END])
            # print(predictions_train[-NOWCASTING_PERIOD:-NOWCASTING_END])
            rmse = np.sqrt(
            np.mean((target[-NOWCASTING_PERIOD:-NOWCASTING_END] -
                     predictions_train[-NOWCASTING_PERIOD:-NOWCASTING_END])**2))
            all_rmses.append(rmse)
            mae_train = np.mean(np.absolute((target[-NOWCASTING_PERIOD:-NOWCASTING_END] -
                     predictions_train[-NOWCASTING_PERIOD:-NOWCASTING_END])))

            plot_predictions_nowcasting(x[0, :, 0].cpu().numpy(), target, predictions_train, 0, 0, state_idx, args)
            
            all_mapes_train.append(np.mean(np.abs((target[-NOWCASTING_PERIOD:-NOWCASTING_END] -
                     predictions_train[-NOWCASTING_PERIOD:-NOWCASTING_END])) / target[-NOWCASTING_PERIOD:-NOWCASTING_END]) * 100)
        ...
        for state_idx, target_values in enumerate(y.unsqueeze(0)):
            ...
            # print(predictions_train.tolist())
        ...
        # 3. Convert the NumPy array to a Pandas DataFrame
        # print(predictions_train.shape)
        # input()
        df = pd.DataFrame(predictions_train, columns=['counts']) # Optional: add column headers
...
def train_predict(args, configs):
    ...
    global MIN_VAL_PARAMS, MAX_VAL_PARAMS, MIN_VAL_PARAMS_2, MAX_VAL_PARAMS_2, NOWCASTING_PERIOD, NOWCASTING_END
    ...
    NOWCASTING_END = configs['nowcasting_period_end']
