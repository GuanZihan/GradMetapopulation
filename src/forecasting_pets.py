...
if __name__ == "__main__":
    ...
    for i in range(n_feature):
        moving_averaged = pd.DataFrame(moving_average(data.iloc[:315, i].ravel(), 7))
        combined_column = pd.concat([moving_averaged, data.iloc[315:, i].reset_index(drop=True)], ignore_index=True)
    ...
    train_X, train_Y = reframed_data[:, :n_in], reframed_data[:, -n_out:][:, 0]
    train_X = train_X.reshape((train_X.shape[0], n_back, n_feature))
    train_Y = train_Y.reshape(-1,n_ahead,1)
